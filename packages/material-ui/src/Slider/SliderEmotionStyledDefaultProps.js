...
const Slider = React.forwardRef(function Slider(props, ref) {
  ...
  const Root = components.root || StyledComponent;
  const rootProps = componentsProps.root || {};

  const Rail = components.rail || StyledRail;
  const railProps = componentsProps.rail || {};

  const Track = components.track || StyledTrack;
  const trackProps = componentsProps.track || {};

  const Thumb = components.thumb || StyledThumb;
  const thumbProps = componentsProps.thumb || {};

  const ValueLabel = components.valueLabel || StyledValueLabel;
  const valueLabelProps = componentsProps.valueLabel || {};

  const Mark = components.mark || StyledMark;
  const markProps = componentsProps.mark || {};

  const MarkLabel = components.markLabel || StyledMarkLabel;
