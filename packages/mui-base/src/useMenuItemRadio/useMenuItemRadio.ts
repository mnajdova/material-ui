...
import type {
  MenuItemRadioMetadata,
  UseMenuItemRadioParameters,
  UseMenuItemRadioReturnValue,
  UseMenuItemRadioRootSlotProps,
} from './useMenuItemRadio.types';
...
function idGenerator(existingKeys: Set<string>) {
  return `menu-item-radio-${existingKeys.size}`;
}
 ...
 * API:
 *
 * - [useMenuItemRadio API](https://mui.com/base-ui/react-menu/hooks-api/#use-menu-item-radio)
 */
export function useMenuItemRadio(params: UseMenuItemRadioParameters): UseMenuItemRadioReturnValue {
  const { disabled = false, checked = false, id: idParam, rootRef: externalRef, label } = params;

  const id = useId(idParam);
  const itemRef = React.useRef<HTMLElement>(null);

  const itemMetadata: MenuItemRadioMetadata = React.useMemo(
  ...
  const { getRootProps: getListRootProps, highlighted, selected } = useListItem({
    item: id,
  });
